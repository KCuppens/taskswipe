...
export async function POST(req: Request) {
  try {
    ...
    // Check if user already has tasks to avoid duplicate seeding
    ...
    if (existingTasks >= 50) {
      return NextResponse.json(
        {
          error: "User already has 50+ tasks. Delete some tasks first or clear all tasks.",
    ...
    return NextResponse.json(
      {
        message: `Successfully created ${createdTasks.length} test tasks`,
...
// Optional: DELETE endpoint to clear all tasks (for testing)
